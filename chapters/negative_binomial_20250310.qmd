...
r_int <- 5L
p_num <- 0.35
NBk_int <- rnbinom(n = 7, size = r_int, prob = p_num)
NBk_int
```

As we can see, in order to achieve 5 wins total in each experiment, we needed `r NBk_int` "games" in those experiments.
...
Now that we have the MGF and a sample of data, we can:
