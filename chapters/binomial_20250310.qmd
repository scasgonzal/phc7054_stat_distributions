...
In other words, if we define $Y = \sum\limits_{i=1}^{n}{X_i}$, where $X_i \sim \text{Ber}(p)$, then $Y \sim \text{Binom}(n,p)$
...
Suppose researchers conduct a clinical trial of a new vaccine. Each participant either **develops immunity** (success) or **does not** (failure). If $(n = 20)$ participants are tested, and each has probability $(p = 0.7)$ of developing immunity, then the number of immune participants, $(X)$, follows a binomial distribution:

$$
X \sim \text{Binom}(n = 20, p = 0.7)
$$
...
5. Suppose $X \sim \text{Binom}(n, p)$ but **both $(n)$ and $p$ are unknown**. Describe the process you would take to estimate $n$. What are the challenges to estimate this parameter if it were truly unknown?
