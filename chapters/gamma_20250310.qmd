...
\end{aligned}
$$
This would require numerical routines to estimate a solution for $\hat{\alpha}$.

The second option is to leave the derivative with respect to $\alpha$ and work with the resulting differential equation. Thankfully this differential equation is in the form $y^{\prime} = C$ with respect to $\alpha$:
$$
\begin{aligned}
0 &\overset{\text{set}}{=} n\log(\lambda) - n \frac{\partial}{\partial\alpha} \log(\Gamma(\alpha)) + n\overline{\log(t)} \\
\Longrightarrow \frac{\partial}{\partial\alpha} \log(\Gamma(\alpha)) &= \log(\lambda) + \overline{\log(t)} \\
\Longrightarrow \log(\Gamma(\alpha)) &= \alpha\left( \log(\lambda) + \alpha\overline{\log(t)} \right) + C.
\end{aligned}
$$
We also need numerical routines to solve this. But the difficult part here is that we don't know what kind of boundary conditions the Gamma Distribution has. Thus, it's very challenging to solve for C here, and any numerical solutions would depend on it. For a discussion of numerical routines related to the MLEs for the Gamma with both parameters unknown, see <https://tminka.github.io/papers/minka-gamma.pdf>.
